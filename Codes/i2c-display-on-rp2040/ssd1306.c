...
// Basic 5x7 font, ASCII 32-127 (space through DEL-1)
...
void ssd1306_draw_char(uint8_t x, uint8_t page, char c) {
    ...
    for (int col = 0; col < 5; col++) {
        uint16_t index = page * SSD1306_WIDTH + x + col;
        if (index < sizeof(framebuffer)) {
            framebuffer[index] = glyph[col];
        }
    }
}
...
void ssd1306_draw_string(uint8_t x, uint8_t page, const char *str) {
    uint8_t cursor_x = x;
    while (*str) {
        ssd1306_draw_char(cursor_x, page, *str);
        cursor_x += 6;  // 5 px glyph + 1 px spacing
        if (cursor_x + 5 >= SSD1306_WIDTH) break; // stop at edge, no wrap
        str++;
    }
}
...
void ssd1306_display(void) {
    ...
    for (int page = 0; page < 8; page++) {
        memcpy(&buf[1], &framebuffer[page * SSD1306_WIDTH], SSD1306_WIDTH);
        i2c_write_blocking(ssd1306_i2c, SSD1306_ADDR, buf, SSD1306_WIDTH + 1, false);
    }
}